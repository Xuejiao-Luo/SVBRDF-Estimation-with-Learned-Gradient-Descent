from rendererMG_util import *

def tex2map(tex):
    # albedo = ((tex[:,0:3,:,:].clamp(-1,1) + 1) / 2) ** 1 #2.2
    ...
    rough = ((tex[:, 5, :, :].clamp(-1, 1) + 1) / 2) ** 1
    rough = rough.clamp(min=eps).unsqueeze(1).expand(-1,3,-1,-1)

    if tex.shape[1] == 9:
        # specular = ((tex[:,6:9,:,:].clamp(-1, 1) + 1) / 2) ** 1 #2.2
...
def png2tex(fn):
    png = Image.open(fn)
    png = png.resize((5*256, 256), resample=Image.BICUBIC)
    png = gyPIL2Array(png) #[0, 1]
...
def png2tex_highlight(fn):
    ...
    img_ndarray = gyPIL2Array(pil_img)

    diffuse = img_ndarray[:, 0:int(newW / 4), :]
    specular = img_ndarray[:, int(newW / 4):2 * int(newW / 4), :]
    roughness = img_ndarray[:, 2 * int(newW / 4):3 * int(newW / 4), 0]
    normals = img_ndarray[:, 3 * int(newW / 4):4 * int(newW / 4), 0:2]
    tex = th.cat((th.from_numpy(diffuse), th.from_numpy(normals), th.from_numpy(roughness).unsqueeze(2), th.from_numpy(specular)), 2)
    tex = tex * 2 - 1
    return tex.permute(2,0,1).unsqueeze(0).cuda(), h

def renderTex(fn_tex, lp, cp, L, fn_im, highlight_flag):

    print(lp, cp, L)
    textures, tex_res = png2tex(fn_tex) if not highlight_flag else png2tex_highlight(fn_tex)
